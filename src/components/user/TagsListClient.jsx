      ...
      {tags.map((tag) => {
        ...
        return (
          <Link
            key={tag._id}
            href={`/tags/${tag.slug}`}
            className={`px-4 py-2 whitespace-nowrap sm:whitespace-normal rounded-full border transition text-sm font-medium shrink-0 cursor-pointer shadow-xs ${
              isActive
          ...
          >
            <span className="pointer-events-none select-none">{tag.name}</span>
          </Link>
        );
      })}
    </div>
