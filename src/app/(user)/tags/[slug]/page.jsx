...
export default async function CategoryPage({ params, searchParams }) {
 const { slug } = await params;
  ...
  const catRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/tags/${slug}`, {
    cache: "no-store",
  });
  const catData = await catRes.json();
  const category = catData?.tag;

  if (!category) {
    return (
      <div className="text-center py-10 text-gray-500">
        Tags not found or removed.
      </div>
    );
  }
  ...
  const productRes = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}/products/search?tags=${category._id}&page=${page}&limit=${limit}`,
    { cache: "no-store" }
  );
